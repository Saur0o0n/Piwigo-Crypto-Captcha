...
if  ((pwg_get_cookie_var('cryptcookietest') == null) and ($_GET[$_GET['sn']]==""))
    {
...
if (is_file($_GET['cfg']) and dirname($_GET['cfg'])=='.' ) pwg_set_session_var('configfile', $_GET['cfg']); 
   else  pwg_set_session_var('configfile', "cryptographp.cfg.php");

include(pwg_get_session_var('configfile'));  


// Vérifie si l'utilisateur a le droit de (re)générer un cryptogramme
if (pwg_get_session_var('cryptcptuse')>=$cryptusemax) {
   header("Content-type: image/png");
   readfile('images/erreur1.png'); 
   exit;
   }

$delai = time()-pwg_get_session_var('crypttime');
...
switch (strtoupper($cryptsecure)) {    
       case "MD5"  : pwg_set_session_var('cryptcode', md5($word)); break;
       case "SHA1" : pwg_set_session_var('cryptcode', sha1($word)); break;
       default     : pwg_set_session_var('cryptcode', $word); break;
       }
pwg_set_session_var('crypttime', time());
pwg_set_session_var('cryptcptuse', pwg_get_session_var('cryptcptuse') +1);       
